...
can be used on different types of resources (subjects, artifacts, volumes,
...
available properties they can add to a flavor or subject. They will see the
...
for subjects). The catalog also includes any additional prefix required when
the property is applied to different types of resources, such as "hw\_" for
subjects and "hw:" for flavors.  So, on an subject, the user would know to set the
...
|  Nova                   | Cinder                    | Glance               |
+=========================+===========================+======================+
| Flavor                  | Volume & Snapshot         | Subject & Snapshot     |
|  + *extra specs*        |  + *subject metadata*       |  + *properties*      |
...
objects, properties, and tags may be used for subjects, snapshots, volumes, and
flavors. Or a namespace may only apply to subjects.
...
The desired virtual CPU topology can be set on both subjects and flavors
via metadata. The keys have different prefixes on subjects than on flavors.
On flavors keys are prefixed with ``hw:``, but on subjects the keys are prefixed
with ``hw_``.
