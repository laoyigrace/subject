...
RELOAD_SERVERS = ['subject-api', 'subject-registry']
GRACEFUL_SHUTDOWN_SERVERS = ['subject-api', 'subject-registry',
                             'subject-scrubber']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/subject/%s.pid' % server):
            pid_files = ['/var/run/subject/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['subject-' + s for s in ALL_SERVERS])
