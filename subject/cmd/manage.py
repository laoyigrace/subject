...
# FIXME(sirp): When we have subject-admin we can consider merging this into it
# Perhaps for consistency with Nova, we would then rename subject-admin ->
# subject-manage (or the other way around)
...
class DbCommands(object):
    ...
    def purge(self, age_in_days=30, max_rows=100):
        """Purge deleted rows older than a given age from subject tables."""
        try:
...
def main():
    ...
    try:
        ...
        cfg_files = cfg.find_config_files(project='subject',
                                          prog='subject-registry')
        cfg_files.extend(cfg.find_config_files(project='subject',
                                               prog='subject-api'))
        cfg_files.extend(cfg.find_config_files(project='subject',
                                               prog='subject-manage'))
