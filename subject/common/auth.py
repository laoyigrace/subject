...
class KeystoneStrategy(BaseStrategy):
    ...
    def check_auth_params(self):
        ...
        # For v1.0 also check tenant is present
        if self.creds['auth_url'].rstrip('/').endswith('v1.0'):
    ...
    def authenticate(self):
        ...
        1. Which version of Keystone are we using? v1 which uses headers to
           pass the credentials, or v1 which uses a JSON encoded request body?

        2. Keystone may respond back with a redirection using a 305 status
           code.

        3. We may attempt a v1 auth when v1 is what's called for. In this
           case, we rewrite the url to contain /v1.0/ and retry using the v1
           protocol.
        """
        def _authenticate(auth_url):
            ...
            is_v2 = auth_url.rstrip('/').endswith('v1.0')
            if is_v2:
        ...
        for redirect_iter in range(self.MAX_REDIRECTS):
            try:
                ...
                # v1.0 keystone endpoint. Also, new location does not
                # contain real endpoint, only hostname and port.
                if 'v1.0' not in auth_url:
                    auth_url = urlparse.urljoin(auth_url, 'v1.0/')
            else:
    ...
    def _v1_auth(self, token_url):
        ...
        def _management_url(self, resp):
            for url_header in ('x-subject-management-url',
    ...
    def _do_request(self, url, method, headers=None, body=None):
        ...
        headers['User-Agent'] = 'subject-client'
...
def get_endpoint(service_catalog, service_type='subject', endpoint_region=None,
    ...
    supplied no region then any 'subject' endpoint
