...
location_strategy_opts = [
    cfg.StrOpt('location_strategy',
               ...
               help=_("""
Strategy to determine the preference order of subject locations.

This configuration option indicates the strategy to determine
the order in which an subject's locations must be accessed to
serve the subject's data. Glance then retrieves the subject data
...
which suggests that subject data be served by using locations in
the order they are stored in Glance. The ``store_type`` value
sets the subject location preference based on the order in which
...
def _load_strategies():
    ...
    modules = {}
    namespace = "subject.common.subject_location_strategy.modules"
...
# TODO(kadachi): Not used but don't remove this until subject_store
...
def get_ordered_locations(locations, **kwargs):
    """
    Order subject location list by configured strategy.
    ...
    :returns: The subject location list with strategy-specific order.
    """
    if not locations:
        return []
...
def choose_best_location(locations, **kwargs):
    """
    Choose best location from subject location list by configured strategy.

    :param locations: The original subject location list.
    :param kwargs: Strategy-specific arguments for under layer strategy module.
    :returns: The best location from subject location list.
    """
