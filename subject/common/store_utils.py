...
from subject.i18n import _LE, _LW
...
def safe_delete_from_backend(context, subject_id, location):
    """
    Given a location, delete an subject from the store and
    ...
    :param location: The subject location entry
    """
    ...
    try:
        ...
        if 'id' in location:
            db_api.get_api().subject_location_delete(context, subject_id,
                                                   location['id'], 'deleted')
        return ret
    except store_api.NotFound:
        msg = _LW('Failed to delete subject %s in store from URI') % subject_id
        LOG.warn(msg)
        ...
        msg = (_LE('Failed to delete subject %(subject_id)s from store: %(exc)s') %
               dict(subject_id=subject_id, exc=exc_type))
        LOG.error(msg)


def schedule_delayed_delete_from_backend(context, subject_id, location):
    """
    Given a location, schedule the deletion of an subject location and
    update location status to db.
    ...
    ret = db_queue.add_location(subject_id, location)
    if ret:
        location['status'] = 'pending_delete'
        if 'id' in location:
            # NOTE(zhiyan): New added subject location entry will has no 'id'
            # field since it has not been saved to DB.
            db_api.get_api().subject_location_delete(context, subject_id,
                                                   location['id'],
                                                   'pending_delete')
        else:
            db_api.get_api().subject_location_add(context, subject_id, location)

    return ret


def delete_subject_location_from_backend(context, subject_id, location):
    """
    Given a location, immediately or schedule the deletion of an subject
    ...
    if CONF.delayed_delete:
        deleted = schedule_delayed_delete_from_backend(context,
                                                       subject_id, location)
    if not deleted:
        # NOTE(zhiyan) If subject metadata has not been saved to DB
        # such as uploading process failure then we can't use
        # location status mechanism to support subject pending delete.
        safe_delete_from_backend(context, subject_id, location)
...
def validate_external_location(uri):
    ...
    :param uri: The URI of external subject location.
    :returns: Whether given URI of external subject location are OK.
    """
    if not uri:
        return False
