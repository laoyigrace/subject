...
IMAGE_META_HEADERS = ['x-subject-meta-location', 'x-subject-meta-size',
                      ...
                      'x-subject-meta-deleted_at', 'x-subject-meta-min_ram',
                      'x-subject-meta-min_disk', 'x-subject-meta-owner',
                      'x-subject-meta-store', 'x-subject-meta-id',
                      'x-subject-meta-protected', 'x-subject-meta-deleted',
                      'x-subject-meta-virtual_size']
...
class LimitingReader(object):
    """
    Reader designed to fail when reading subject data past the configured
    allowable amount.
    """
    def __init__(self, data, limit):
        """
        :param data: Underlying subject data object
...
def subject_meta_to_http_headers(subject_meta):
    """
    Returns a set of subject metadata into a dict
    of HTTP headers that can be fed to either a Webob
    Request object or an httplib.HTTP(S)Connection object

    :param subject_meta: Mapping of subject metadata
    """
    headers = {}
        ...
        if v is not None:
            if k == 'properties':
                for pk, pv in v.items():
                    if pv is not None:
                        headers["x-subject-meta-property-%s"
                                % pk.lower()] = six.text_type(pv)
            else:
                headers["x-subject-meta-%s" % k.lower()] = six.text_type(v)
    return headers


def get_subject_meta_from_headers(response):
    """
    Processes HTTP headers from a supplied response that
    match the x-subject-meta and x-subject-meta-property and
    returns a mapping of subject metadata and properties
    ...
    for key, value in headers:
        key = str(key.lower())
        if key.startswith('x-subject-meta-property-'):
            field_name = key[len('x-subject-meta-property-'):].replace('-', '_')
            properties[field_name] = value or None
        elif key.startswith('x-subject-meta-'):
            field_name = key[len('x-subject-meta-'):].replace('-', '_')
            if 'x-subject-meta-' + field_name not in IMAGE_META_HEADERS:
        ...
        if key in result:
            try:
                ...
                if nullable and result[key] == str(None):
                    ...
                    extra = (_("Cannot convert subject %(key)s '%(value)s' "
                               "to an integer.")
...
def create_mashup_dict(subject_meta):
    """
    Returns a dictionary-like mashup of the subject core properties
    and the subject custom properties from given subject metadata.

    :param subject_meta: metadata of subject with core and custom properties
    """

    d = {}
    for key, value in six.iteritems(subject_meta):
        if isinstance(value, dict):
            for subkey, subvalue in six.iteritems(
                    create_mashup_dict(value)):
                if subkey not in subject_meta:
                    d[subkey] = subvalue
        else:
            d[key] = value
