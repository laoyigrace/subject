...
"""
This migration handles migrating encrypted subject location values from
the unquoted form to the quoted form.

If 'metadata_encryption_key' is specified in the config then this
migration performs the following steps for every entry in the subjects table:
...
def migrate_location_credentials(migrate_engine, to_quoted):
    ...
    subjects = list(subjects_table.select().execute())

    for subject in subjects:
        try:
            fixed_uri = fix_uri_credentials(subject['location'], to_quoted)
            subjects_table.update().where(
                subjects_table.c.id == subject['id']).values(
                    location=fixed_uri).execute()
        except exception.Invalid:
            msg = _LW("Failed to decrypt location value for subject"
                      " %(subject_id)s") % {'subject_id': subject['id']}
            LOG.warn(msg)
        except exception.BadStoreUri as e:
            reason = encodeutils.exception_to_unicode(e)
            msg = _LE("Invalid store uri for subject: %(subject_id)s. "
                      "Details: %(reason)s") % {'subject_id': subject.id,
                                                'reason': reason}
            LOG.exception(msg)
            raise
