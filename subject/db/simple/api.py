...
def _subject_location_format(subject_id, value, meta_data, status, deleted=False):
    dt = timeutils.utcnow()
    return {
        'id': str(uuid.uuid4()),
...
def _subject_property_format(subject_id, name, value):
    return {
        'subject_id': subject_id,
        'name': name,
        'value': value,
...
def _subject_member_format(subject_id, tenant_id, can_share, status='pending',
                         deleted=False):
    dt = timeutils.utcnow()
    return {
        'id': str(uuid.uuid4()),
...
@utils.no_4byte_params
def _subject_update(subject, values, properties):
    # NOTE(bcwaldon): store properties as a list to match sqlalchemy driver
    properties = [{'name': k,
                   'value': v,
                   'subject_id': subject['id'],
                   'deleted': False} for k, v in properties.items()]
    if 'properties' not in subject.keys():
        subject['properties'] = []
    subject['properties'].extend(properties)
    subject.update(values)
    return subject


def _subject_format(subject_id, **values):
    dt = timeutils.utcnow()
    subject = {
        'id': subject_id,
        'name': None,
    ...
    if locations is not None:
        ...
        for location in locations:
            location_ref = _subject_location_format(subject_id,
                                                  location['url'],
                                                  location['metadata'],
                                                  location['status'])
            subject['locations'].append(location_ref)
            DATA['locations'].append(location_ref)

    return _subject_update(subject, values, values.pop('properties', {}))


def _filter_subjects(subjects, filters, context,
    ...
    for subject in subjects:
        member = subject_member_find(context, subject_id=subject['id'],
                                   member=context.owner, status=status)
        is_member = len(member) > 0
        has_ownership = context.owner and subject['owner'] == context.owner
        can_see = (subject['is_public'] or has_ownership or is_member or
        ...
        if is_public is not None:
            if not subject['is_public'] == is_public:
                continue
        ...
        for k, value in six.iteritems(filters):
            ...
            elif k != 'is_public' and subject.get(k) is not None:
                to_add = subject.get(key) == value
            elif k == 'tags':
                filter_tags = value
                subject_tags = subject_tag_get_all(context, subject['id'])
...
def _do_pagination(context, subjects, marker, limit, show_deleted,
                   status='accepted'):
    start = 0
    end = -1
    if marker is None:
        ...
        # Check that the subject is accessible
        _subject_get(context, marker, force_show_deleted=show_deleted,
                   status=status)

        for i, subject in enumerate(subjects):
            if subject['id'] == marker:
                start = i + 1
                break
        else:
...
def _sort_subjects(subjects, sort_key, sort_dir):
    ...
    for key in sort_key:
        if subjects and not (key in subjects[0]):
    ...
    for key, dir in reversed(list(zip(sort_key, sort_dir))):
        reverse = dir == 'desc'
        subjects.sort(key=lambda x: x[key] or '', reverse=reverse)

    return subjects


def _subject_get(context, subject_id, force_show_deleted=False, status=None):
    try:
    ...
    if not is_subject_visible(context, subject):
        ...
        raise exception.Forbidden("Subject not visible to you")
...
def subject_get(context, subject_id, session=None, force_show_deleted=False):
    subject = _subject_get(context, subject_id, force_show_deleted)
    return _normalize_locations(context, copy.deepcopy(subject),
...
def subject_get_all(context, filters=None, marker=None, limit=None,
    ...
    subjects = _filter_subjects(subjects, filters, context, member_status,
                            is_public, admin_as_user)
    subjects = _sort_subjects(subjects, sort_key, sort_dir)
    subjects = _do_pagination(context, subjects, marker, limit,
                            filters.get('deleted'))

    force_show_deleted = True if filters.get('deleted') else False
    res = []
    for subject in subjects:
        img = _normalize_locations(context, copy.deepcopy(subject),
                                   force_show_deleted=force_show_deleted)
        if return_tag:
            img['tags'] = subject_tag_get_all(context, img['id'])
        res.append(img)
    return res


@log_call
def subject_property_create(context, values):
    subject = _subject_get(context, values['subject_id'])
    prop = _subject_property_format(values['subject_id'],
                                  values['name'],
                                  values['value'])
    subject['properties'].append(prop)
    return prop


@log_call
def subject_property_delete(context, prop_ref, subject_ref):
    prop = None
    for p in DATA['subjects'][subject_ref]['properties']:
        if p['name'] == prop_ref:
            prop = p
    if not prop:
...
def subject_member_find(context, subject_id=None, member=None,
    ...
    def is_visible(member):
        return (member['member'] == context.owner or
                subjects[member['subject_id']]['owner'] == context.owner)

    if not context.is_admin:
        filters.append(is_visible)

    if subject_id is not None:
        filters.append(lambda m: m['subject_id'] == subject_id)
...
def subject_member_count(context, subject_id):
    ...
    members = DATA['members']
    return len([x for x in members if x['subject_id'] == subject_id])


@log_call
def subject_member_create(context, values):
    member = _subject_member_format(values['subject_id'],
                                  values['member'],
...
@log_call
def subject_member_update(context, member_id, values):
    global DATA
...
@log_call
def subject_member_delete(context, member_id):
    global DATA
...
@log_call
@utils.no_4byte_params
def subject_location_add(context, subject_id, location):
    ...
    DATA['locations'].append(location_ref)
    subject = DATA['subjects'][subject_id]
    subject.setdefault('locations', []).append(location_ref)


@log_call
@utils.no_4byte_params
def subject_location_update(context, subject_id, location):
    ...
    for loc in DATA['locations']:
        if loc['id'] == loc_id and loc['subject_id'] == subject_id:
...
def subject_location_delete(context, subject_id, location_id, status,
    ...
    deleted = False
    for loc in DATA['locations']:
        if loc['id'] == location_id and loc['subject_id'] == subject_id:
            deleted = True
...
def _subject_locations_set(context, subject_id, locations):
    ...
    for loc in subject['locations']:
        if loc['id'] not in used_loc_ids and not loc['deleted']:
            subject_location_delete(context, subject_id, loc['id'], 'deleted')
    for i, loc in enumerate(DATA['locations']):
        if (loc['subject_id'] == subject_id and loc['id'] not in used_loc_ids and
                not loc['deleted']):
    ...
    for loc in locations:
        if loc.get('id') is None:
            subject_location_add(context, subject_id, loc)
        else:
            subject_location_update(context, subject_id, loc)


def _subject_locations_delete_all(context, subject_id, delete_time=None):
    subject = DATA['subjects'][subject_id]
    for loc in subject['locations']:
        if not loc['deleted']:
            subject_location_delete(context, subject_id, loc['id'], 'deleted',
    ...
    for i, loc in enumerate(DATA['locations']):
        if subject_id == loc['subject_id'] and loc['deleted'] == False:
            del DATA['locations'][i]
...
@log_call
def subject_create(context, subject_values):
    global DATA
    subject_id = subject_values.get('id', str(uuid.uuid4()))

    if subject_id in DATA['subjects']:
        raise exception.Duplicate()

    if 'status' not in subject_values:
    ...
    incorrect_keys = set(subject_values.keys()) - allowed_keys
    if incorrect_keys:
        raise exception.Invalid(
            'The keys %s are not valid' % str(incorrect_keys))

    subject = _subject_format(subject_id, **subject_values)
    DATA['subjects'][subject_id] = subject
    DATA['tags'][subject_id] = subject.pop('tags', [])

    return _normalize_locations(context, copy.deepcopy(subject))


@log_call
def subject_update(context, subject_id, subject_values, purge_props=False,
                 from_state=None):
    global DATA
    try:
        subject = DATA['subjects'][subject_id]
    ...
    if location_data is not None:
        _subject_locations_set(context, subject_id, location_data)

    # replace values for properties that already exist
    new_properties = subject_values.pop('properties', {})
    ...
    subject['updated_at'] = timeutils.utcnow()
    _subject_update(subject, subject_values, new_properties)
    DATA['subjects'][subject_id] = subject
    return _normalize_locations(context, copy.deepcopy(subject))
...
def subject_destroy(context, subject_id):
    ...
    try:
        ...
        DATA['subjects'][subject_id]['deleted'] = True
        DATA['subjects'][subject_id]['deleted_at'] = delete_time

        # NOTE(flaper87): Move the subject to one of the deleted statuses
        # if it hasn't been done yet.
        if (DATA['subjects'][subject_id]['status'] not in
                ['deleted', 'pending_delete']):
            DATA['subjects'][subject_id]['status'] = 'deleted'

        _subject_locations_delete_all(context, subject_id,
                                    delete_time=delete_time)

        for prop in DATA['subjects'][subject_id]['properties']:
            subject_property_delete(context, prop['name'], subject_id)

        members = subject_member_find(context, subject_id=subject_id)
        for member in members:
            subject_member_delete(context, member['id'])

        tags = subject_tag_get_all(context, subject_id)
        for tag in tags:
            subject_tag_delete(context, subject_id, tag)
...
@log_call
def subject_tag_get_all(context, subject_id):
    return DATA['tags'].get(subject_id, [])


@log_call
def subject_tag_get(context, subject_id, value):
    tags = subject_tag_get_all(context, subject_id)
    if value in tags:
        return value
    else:
...
@log_call
def subject_tag_set_all(context, subject_id, values):
    global DATA
    DATA['tags'][subject_id] = list(values)


@log_call
@utils.no_4byte_params
def subject_tag_create(context, subject_id, value):
    global DATA
    DATA['tags'][subject_id].append(value)
    return value


@log_call
def subject_tag_delete(context, subject_id, value):
    global DATA
    try:
        DATA['tags'][subject_id].remove(value)
    except ValueError:
        raise exception.NotFound()
...
def user_get_storage_usage(context, owner_id, subject_id=None, session=None):
    subjects = subject_get_all(context, filters={'owner': owner_id})
    total = 0
    for subject in subjects:
        if subject['status'] in ['killed', 'deleted']:
            continue

        if subject['id'] != subject_id:
            locations = [loc for loc in subject['locations']
                         if loc.get('status') != 'deleted']
            total += (subject['size'] * len(locations))
    return total
...
def _artifact_get(context, artifact_id, type_name,
    ...
    if artifact['deleted_at']:
        LOG.info(_LI('Unable to get deleted subject'))
        raise exception.NotFound()
