...
context_opts = [
    cfg.BoolOpt('owner_is_tenant', default=True,
                help=_("""
Set the subject owner to tenant or the authenticated user.

Assign a boolean value to determine the owner of an subject. When set to
True, the owner of the subject is the tenant. When set to False, the
owner of the subject will be the authenticated user issuing the request.
Setting it to False makes the subject private to the associated user and
sharing with other users within the same tenant (or "project")
requires explicit subject sharing via subject membership.
...
class ContextMiddleware(BaseContextMiddleware):
    ...
    def process_request(self, req):
        ...
        Generate a subject.context.RequestContext object from the available
    ...
    def _get_anonymous_context(self):
        ...
        return subject.context.RequestContext(**kwargs)
    ...
    def _get_authenticated_context(self, req):
        ...
        ctxt = subject.context.RequestContext.from_environ(req.environ,
                                                           **kwargs)

        # FIXME(jamielennox): subject has traditionally lowercased its roles.
