...
from subject.common import client as base_client
...
class CacheClient(base_client.BaseClient):
    ...
    def delete_cached_subject(self, subject_id):
        """
        Delete a specified subject from the cache
        """
        self.do_request("DELETE", "/cached_subjects/%s" % subject_id)
        return True

    def get_cached_subjects(self, **kwargs):
        """
        Returns a list of subjects stored in the subject cache.
        """
        res = self.do_request("GET", "/cached_subjects")
        data = json.loads(res.read())['cached_subjects']
        return data

    def get_queued_subjects(self, **kwargs):
        """
        Returns a list of subjects queued for caching
        """
        res = self.do_request("GET", "/queued_subjects")
        data = json.loads(res.read())['queued_subjects']
        return data
    ...
    def delete_all_cached_subjects(self):
        ...
        """
        res = self.do_request("DELETE", "/cached_subjects")
        data = json.loads(res.read())
        num_deleted = data['num_deleted']
        return num_deleted

    def queue_subject_for_caching(self, subject_id):
        """
        Queue an subject for prefetching into cache
        """
        self.do_request("PUT", "/queued_subjects/%s" % subject_id)
        return True

    def delete_queued_subject(self, subject_id):
        """
        Delete a specified subject from the cache queue
        """
        self.do_request("DELETE", "/queued_subjects/%s" % subject_id)
        return True
    ...
    def delete_all_queued_subjects(self):
        ...
        """
        res = self.do_request("DELETE", "/queued_subjects")
        data = json.loads(res.read())
        num_deleted = data['num_deleted']
        return num_deleted
