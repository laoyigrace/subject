...
from subject.domain import proxy as domain_proxy
from subject.i18n import _, _LE


notifier_opts = [
    cfg.StrOpt('default_publisher_id',
               default="subject.localhost",
               help=_("""
...
Possible values:
    * Any reasonable instance identifier, for example: subject.host1
...
Possible values:
    ...
    http://docs.openstack.org/developer/subject/notifications.html

    The values must be specified as: <group_name>.<event_name>
    For example: subject.create,task.success,metadef_tag
...
def set_defaults(control_exchange='subject'):
...
def format_subject_notification(subject):
    """
    Given a subject.domain.Subject object, return a dictionary of relevant
    ...
    return {
        ...
        'created_at': timeutils.isotime(subject.created_at),
        'updated_at': timeutils.isotime(subject.updated_at),
...
def format_subject_member_notification(subject_member):
    ...
    """
    return {
        'subject_id': subject_member.subject_id,
        'member_id': subject_member.member_id,
        'status': subject_member.status,
        'created_at': timeutils.isotime(subject_member.created_at),
        'updated_at': timeutils.isotime(subject_member.updated_at),
        'deleted': False,
        'deleted_at': None,
    }
...
class ImageProxy(NotificationProxy, domain_proxy.Subject):
    ...
    def _format_subject_send(self, bytes_sent):
        return {
            'bytes_sent': bytes_sent,
            'subject_id': self.repo.subject_id,
    ...
    def _get_chunk_data_iterator(self, data, chunk_size=None):
        ...
        try:
            _send_notification(notify, 'subject.send',
                               self._format_subject_send(sent))
    ...
    def set_data(self, data, size=None):
        self.send_notification('subject.prepare', self.repo)
        ...
        try:
            ...
            msg = (_("Cannot save data for subject %(subject_id)s: %(error)s") %
                   {'subject_id': self.repo.subject_id,
            ...
            msg = (_("Unable to upload duplicate subject data for subject"
            ...
            msg = (_("Not allowed to upload subject data for subject %(subject_id)s:"
                     " %(error)s")
                   % {'subject_id': self.repo.subject_id,
                      'error': encodeutils.exception_to_unicode(e)})
            _send_notification(notify_error, 'subject.upload', msg)
            ...
            msg = (_("Subject %(subject_id)s could not be found after upload."
                     " The subject may have been deleted during the upload:"
                     " %(error)s") % {'subject_id': self.repo.subject_id,
                                      'error': exc_str})
            _send_notification(notify_error, 'subject.upload', msg)
            raise webob.exc.HTTPNotFound(explanation=exc_str)
        except webob.exc.HTTPError as e:
            with excutils.save_and_reraise_exception():
                msg = (_("Failed to upload subject data for subject %(subject_id)s"
            ...
            self.send_notification('subject.upload', self.repo)
...
class SubjectRepoProxy(NotificationRepoProxy, domain_proxy.Repo):
    ...
    def save(self, subject, from_state=None):
        super(SubjectRepoProxy, self).save(subject, from_state=from_state)
        self.send_notification('subject.update', subject)

    def add(self, subject):
        super(SubjectRepoProxy, self).add(subject)
        self.send_notification('subject.create', subject)

    def remove(self, subject):
        super(SubjectRepoProxy, self).remove(subject)
        self.send_notification('subject.delete', subject, extra_payload={
...
class ImageMemberRepoProxy(NotificationBase, domain_proxy.MemberRepo):
    ...
    def __init__(self, repo, subject, context, notifier):
        ...
        proxy_class = self.get_proxy_class()
        super_class = self.get_super_class()
        super_class.__init__(self, subject, repo, proxy_class, proxy_kwargs)
