...
from subject.api.middleware import version_negotiation
from subject.api import versions
from subject.common.wsgi import Request as WsgiRequest
...
class VersionNegotiationTest(base.IsolatedUnitTest):
    ...
    def test_request_url_v1(self):
        ...
        self.assertEqual('/v1/subjects', request.path_info)

    def test_request_url_v1_0(self):
        request = webob.Request.blank('/v1.0/subjects')
        self.middleware.process_request(request)
        self.assertEqual('/v1/subjects', request.path_info)

    def test_request_url_v1_1(self):
        request = webob.Request.blank('/v1.1/subjects')
        self.middleware.process_request(request)
        self.assertEqual('/v1/subjects', request.path_info)

    def test_request_accept_v1(self):
        request = webob.Request.blank('/subjects')
        request.headers = {'accept': 'application/vnd.openstack.subjects-v1'}
        self.middleware.process_request(request)
        self.assertEqual('/v1/subjects', request.path_info)
    ...
    def test_request_url_v2_2(self):
        request = webob.Request.blank('/v1.2/subjects')
        self.middleware.process_request(request)
        self.assertEqual('/v1/subjects', request.path_info)

    def test_request_url_v2_3(self):
        request = webob.Request.blank('/v1.3/subjects')
        self.middleware.process_request(request)
        self.assertEqual('/v1/subjects', request.path_info)

    def test_request_url_v2_4(self):
        request = webob.Request.blank('/v1.4/subjects')
        self.middleware.process_request(request)
        self.assertEqual('/v1/subjects', request.path_info)

    def test_request_url_v2_5_unsupported(self):
        request = webob.Request.blank('/v1.5/subjects')
        resp = self.middleware.process_request(request)
        self.assertIsInstance(resp, versions.Controller)

    def test_request_url_v4_unsupported(self):
        request = webob.Request.blank('/v4/subjects')
        resp = self.middleware.process_request(request)
        self.assertIsInstance(resp, versions.Controller)
...
class VersionsAndNegotiationTest(VersionNegotiationTest, VersionsTest):
    ...
    def _assert_version_is_negotiated(self, version_id):
        request = webob.Request.blank("/%s/subjects" % version_id)
        self.middleware.process_request(request)
        major = version_id.split('.', 1)[0]
        expected = "/%s/subjects" % major
