...
class TestMetadefRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = subject.context.RequestContext(user=USER1,
                                                      tenant=TENANT1)
        self.namespace_repo = subject.db.MetadefNamespaceRepo(self.context,
                                                              self.db)
        self.property_repo = subject.db.MetadefPropertyRepo(self.context,
                                                            self.db)
        self.object_repo = subject.db.MetadefObjectRepo(self.context,
                                                        self.db)
        self.tag_repo = subject.db.MetadefTagRepo(self.context,
                                                  self.db)
        self.resource_type_repo = subject.db.MetadefResourceTypeRepo(
            self.context, self.db)
        self.namespace_factory = subject.domain.MetadefNamespaceFactory()
        self.property_factory = subject.domain.MetadefPropertyFactory()
        self.object_factory = subject.domain.MetadefObjectFactory()
        self.tag_factory = subject.domain.MetadefTagFactory()
        self.resource_type_factory = subject.domain.MetadefResourceTypeFactory()
