...
class TestUploadUtils(base.StoreClearingUnitTest):
    ...
    def test_safe_kill(self):
        ...
        with patch.object(registry, "update_subject_metadata") as mock_registry:
    ...
    @contextmanager
    def _get_store_and_notifier(self, subject_size=10, ext_update_data=None,
        ...
        subject_meta = {'id': unit_test_utils.UUID1,
                      'size': subject_size}
        subject_data = "blah"
        ...
        yield (location, checksum, subject_meta, subject_data, store, notifier,
               update_data)

    def test_upload_data_to_store(self):
        # 'user_storage_quota' is not set
        def store_add(subject_id, data, size, **kwargs):
        ...
        with self._get_store_and_notifier(
                ...
                exc_class=store_add) as (location, checksum, subject_meta,
                                         subject_data, store, notifier,
                                         update_data):
            ret = subject_meta.update(update_data)
            with patch.object(registry, 'update_subject_metadata',
                              return_value=ret) as mock_update_subject_metadata:
                actual_meta, location_data = upload_utils.upload_data_to_store(
                    req, subject_meta, subject_data, store, notifier)

                self.assertEqual(location, location_data['url'])
                self.assertEqual(subject_meta.update(update_data), actual_meta)
                mock_update_subject_metadata.assert_called_once_with(
                    req.context, subject_meta['id'], update_data,
                    from_state='saving')
    ...
    def test_upload_data_to_store_user_storage_quota_enabled(self):
        ...
        with self._get_store_and_notifier(
            ...
            ret = subject_meta.update(update_data)
            # mock 'check_quota'
            mock_check_quota = patch('subject.api.common.check_quota',
                                     return_value=100)
    ...
    def test_upload_data_to_store_mismatch_size(self):
        ...
        with self._get_store_and_notifier(
            subject_size=11) as (location, checksum, subject_meta, subject_data,
            ...
            with patch.object(registry, 'update_subject_metadata',
                ...
                self.assertRaises(webob.exc.HTTPBadRequest,
                                  ...
                                  req, subject_meta, subject_data, store,
                                  notifier)
                mock_update_subject_metadata.assert_called_with(
                    req.context, subject_meta['id'], {'status': 'killed'},
                    from_state='saving')
    ...
    def test_upload_data_to_store_mismatch_checksum(self):
        ...
        with self._get_store_and_notifier(
            ret_checksum='fake') as (location, checksum, subject_meta,
                                     subject_data, store, notifier, update_data):
            ret = subject_meta.update(update_data)
            with patch.object(registry, "update_subject_metadata",
                              return_value=ret) as mock_update_subject_metadata:
                self.assertRaises(webob.exc.HTTPBadRequest,
    ...
    def _test_upload_data_to_store_exception(self, exc_class, expected_class):
        ...
        with self._get_store_and_notifier(
            exc_class=exc_class) as (location, checksum, subject_meta,
                                     subject_data, store, notifier, update_data):
        ...
        with self._get_store_and_notifier(
            ...
            with patch.object(upload_utils, 'safe_kill') as mock_safe_kill:
                self.assertRaises(expected_class,
                                  ...
                                  notifier)
                if subject_killed:
                    mock_safe_kill.assert_called_with(req, subject_meta['id'],
                                                      'saving')
    ...
    def test_upload_data_to_store_duplicate(self):
        """See note in subject.api.v1.upload_utils on why we don't want subject to
        be deleted in this case.
        """
        self._test_upload_data_to_store_exception_with_notify(
            exception.Duplicate,
            webob.exc.HTTPConflict,
            subject_killed=False)
    ...
    def test_activate_subject_with_expired_token(
            self, mocked_delete, mocked_update):
        """Test token expiration during subject upload.

        If users token expired before subject was uploaded then if auth error
        was caught from registry during changing subject status from 'saving'
        to 'active' then it's required to delete all subject data.
        """
        context = mock.Mock()
        req = mock.Mock()
        req.context = context
        with self._get_store_and_notifier() as (location, checksum, subject_meta,
                                                subject_data, store, notifier,
                                                update_data):
