...
from subject.tests.unit import utils as unit_utils
from subject.tests import utils


def _fake_subject(owner, is_public):
    return {
        'id': None,
        'owner': owner,
        'is_public': is_public,
...
class TestContext(utils.BaseTestCase):
    ...
    def do_visible(self, exp_res, img_owner, img_public, **kwargs):
        """
        Perform a context visibility test.  Creates a (fake) subject
        with the specified owner and is_public attributes, then
        creates a context with the given keyword arguments and expects
        exp_res as the result of an is_subject_visible() call on the
        context.
        """

        img = _fake_subject(img_owner, img_public)
        ctx = context.RequestContext(**kwargs)

        self.assertEqual(exp_res, self.db_api.is_subject_visible(ctx, img))

    def test_empty_public(self):
        """
        Tests that an empty context (with is_admin set to True) can
        access an subject with is_public set to True.
        """
    ...
    def test_empty_public_owned(self):
        """
        Tests that an empty context (with is_admin set to True) can
        access an owned subject with is_public set to True.
        """
    ...
    def test_empty_private(self):
        """
        Tests that an empty context (with is_admin set to True) can
        access an subject with is_public set to False.
        """
    ...
    def test_empty_private_owned(self):
        """
        Tests that an empty context (with is_admin set to True) can
        access an owned subject with is_public set to False.
        """
    ...
    def test_anon_public(self):
        ...
        can access an subject with is_public set to True.
        """
    ...
    def test_anon_public_owned(self):
        """
        Tests that an anonymous context (with is_admin set to False)
        can access an owned subject with is_public set to True.
        """
    ...
    def test_anon_private(self):
        """
        Tests that an anonymous context (with is_admin set to False)
        can access an unowned subject with is_public set to False.
        """
    ...
    def test_anon_private_owned(self):
        """
        Tests that an anonymous context (with is_admin set to False)
        cannot access an owned subject with is_public set to False.
        """
    ...
    def test_auth_public(self):
        ...
        False) can access an subject with is_public set to True.
        """
    ...
    def test_auth_public_unowned(self):
        """
        Tests that an authenticated context (with is_admin set to
        False) can access an subject (which it does not own) with
    ...
    def test_auth_public_owned(self):
        """
        Tests that an authenticated context (with is_admin set to
        False) can access an subject (which it does own) with is_public
        set to True.
        """
    ...
    def test_auth_private(self):
        """
        Tests that an authenticated context (with is_admin set to
        False) can access an subject with is_public set to False.
        """
    ...
    def test_auth_private_unowned(self):
        """
        Tests that an authenticated context (with is_admin set to
        False) cannot access an subject (which it does not own) with
