...
from subject.common import rpc
...
class FakeResource(object):
    ...
    def get_subjects(self, context, keyword=None):
        return keyword

    def count_subjects(self, context, subjects):
        return len(subjects)

    def get_all_subjects(self, context):
        return False
...
class TestRPCController(base.IsolatedUnitTest):
    ...
    def test_register(self):
        ...
        self.assertIn("get_subjects", controller._registered)
        self.assertIn("get_all_subjects", controller._registered)

    def test_reigster_filtered(self):
        res = FakeResource()
        controller = rpc.Controller()
        controller.register(res, filtered=["get_all_subjects"])
        self.assertIn("get_all_subjects", controller._registered)

    def test_reigster_excluded(self):
        res = FakeResource()
        controller = rpc.Controller()
        controller.register(res, excluded=["get_all_subjects"])
        self.assertIn("get_subjects", controller._registered)
    ...
    def test_reigster_refiner(self):
        ...
        self.assertRaises(TypeError,
                          controller.register,
                          res, refiner="get_all_subjects")

        # Filter returns False
        controller.register(res, refiner=lambda x: False)
        self.assertNotIn("get_subjects", controller._registered)
        self.assertNotIn("get_subjects", controller._registered)
    ...
    def test_rpc_exception_propagation(self):
        ...
        returned = jsonutils.loads(res.body)[0]
        self.assertEqual('subject.common.exception.RPCError',
                         returned['_error']['cls'])
...
class TestRPCClient(base.IsolatedUnitTest):
    ...
    def test_bulk_request(self):
        commands = [{"command": "get_subjects", 'kwargs': {'keyword': True}},
                    {"command": "get_all_subjects"}]
    ...
    def test_non_str_or_dict_response(self):
        rst = self.client.count_subjects(subjects=[1, 2, 3, 4])
