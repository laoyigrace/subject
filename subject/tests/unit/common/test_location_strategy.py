...
from subject.common.location_strategy import location_order
from subject.common.location_strategy import store_type
from subject.tests.unit import base


class TestLocationStrategy(base.IsolatedUnitTest):
    """Test routines in subject.common.location_strategy"""
...
class TestLocationOrderStrategyModule(base.IsolatedUnitTest):
    """Test routines in subject.common.location_strategy.location_order"""
...
class TestStoreTypeStrategyModule(base.IsolatedUnitTest):
    """Test routines in subject.common.location_strategy.store_type"""
    ...
    def test_get_ordered_locations(self):
        ...
        locs = [{'url': 'file://subject0', 'metadata': {'idx': 3}},
                {'url': 'rbd://subject1', 'metadata': {'idx': 0}},
                {'url': 'file://subject3', 'metadata': {'idx': 4}},
                {'url': 'swift://subject4', 'metadata': {'idx': 6}},
                {'url': 'cinder://subject5', 'metadata': {'idx': 9}},
                {'url': 'file://subject6', 'metadata': {'idx': 5}},
                {'url': 'rbd://subject7', 'metadata': {'idx': 1}},
                {'url': 'vsphere://subject9', 'metadata': {'idx': 8}},
                {'url': 'sheepdog://subject8', 'metadata': {'idx': 2}}]
    ...
    def test_get_ordered_locations_with_invalid_store_name(self):
        self.config(store_type_preference=['  rbd', 'sheepdog ', 'invalid',
                                           'swift  ', '  http  '],
                    group='store_type_location_strategy')
        locs = [{'url': 'file://subject0', 'metadata': {'idx': 4}},
                {'url': 'rbd://subject1', 'metadata': {'idx': 0}},
                {'url': 'file://subject3', 'metadata': {'idx': 5}},
                {'url': 'swift://subject4', 'metadata': {'idx': 3}},
                {'url': 'cinder://subject5', 'metadata': {'idx': 6}},
                {'url': 'file://subject6', 'metadata': {'idx': 7}},
                {'url': 'rbd://subject7', 'metadata': {'idx': 1}},
                {'url': 'sheepdog://subject8', 'metadata': {'idx': 2}}]
        ordered_locs = store_type.get_ordered_locations(copy.deepcopy(locs))
        locs.sort(key=lambda loc: loc['metadata']['idx'])
    ...
    def test_get_ordered_locations_with_consistent_store_names(self):
        ...
        in subject to be consistent with store names used in glance_store.
