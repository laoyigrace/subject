...
from subject.api.v1 import router as router_v1
from subject.api.v2 import router as router_v2
...
class RequestTest(test_utils.BaseTestCase):
    ...
    def _set_expected_languages(self, all_locales=None, avail_locales=None):
        ...
        def fake_gettext_find(lang_id, *args, **kwargs):
            found_ret = '/subject/%s/LC_MESSAGES/subject.mo' % lang_id
    ...
    def test_http_error_response_codes(self):
        ...
        unallowed_methods = [
            ('/subjects', ['PUT', 'DELETE', 'HEAD', 'PATCH']),
            ('/subjects/detail', ['POST', 'PUT', 'DELETE', 'PATCH']),
            ('/subjects/%s' % sample_id, ['POST', 'PATCH']),
            ('/subjects/%s/members' % sample_id,
                ['POST', 'DELETE', 'HEAD', 'PATCH']),
            ('/subjects/%s/members/%s' % (sample_id, member_id),
                ['POST', 'HEAD', 'PATCH']),
        ]
        ...
        """Makes sure v1 unallowed methods return 405"""
        unallowed_methods = [
            ('/schemas/subject', ['POST', 'PUT', 'DELETE', 'PATCH', 'HEAD']),
            ('/schemas/subjects', ['POST', 'PUT', 'DELETE', 'PATCH', 'HEAD']),
            ...
            ('/subjects', ['PUT', 'DELETE', 'PATCH', 'HEAD']),
            ('/subjects/%s' % sample_id, ['POST', 'PUT', 'HEAD']),
            ('/subjects/%s/file' % sample_id,
                ['POST', 'DELETE', 'PATCH', 'HEAD']),
            ('/subjects/%s/tags/%s' % (sample_id, tag_val),
                ['GET', 'POST', 'PATCH', 'HEAD']),
            ('/subjects/%s/members' % sample_id,
        ...
        req = webob.Request.blank('/schemas/subject')
...
class TestHelpers(test_utils.BaseTestCase):
    ...
    def test_headers_are_unicode(self):
        ...
        headers = utils.subject_meta_to_http_headers(fixture)
    ...
    def test_data_passed_properly_through_headers(self):
        ...
        result = utils.get_subject_meta_from_headers(response)
...
class GetSocketTestCase(test_utils.BaseTestCase):
    ...
    def test_correct_configure_socket(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
            'subject.common.wsgi.ssl.wrap_socket',
            mock_socket))
        self.useFixture(fixtures.MonkeyPatch(
            'subject.common.wsgi.eventlet.listen',
