...
class TestUtils(test_utils.BaseTestCase):
    """Test routines in subject.utils"""
    ...
    def test_get_meta_from_headers(self):
        resp = webob.Response()
        resp.headers = {"x-subject-meta-name": 'test',
                        'x-subject-meta-virtual-size': 80}
        result = utils.get_subject_meta_from_headers(resp)
    ...
    def test_get_meta_from_headers_none_virtual_size(self):
        resp = webob.Response()
        resp.headers = {"x-subject-meta-name": 'test',
                        'x-subject-meta-virtual-size': 'None'}
        result = utils.get_subject_meta_from_headers(resp)
        self.assertEqual({'name': 'test', 'properties': {},
    ...
    def test_get_meta_from_headers_bad_headers(self):
        resp = webob.Response()
        resp.headers = {"x-subject-meta-bad": 'test'}
        self.assertRaises(webob.exc.HTTPBadRequest,
                          utils.get_subject_meta_from_headers, resp)
        resp.headers = {"x-subject-meta-": 'test'}
        self.assertRaises(webob.exc.HTTPBadRequest,
                          utils.get_subject_meta_from_headers, resp)
        resp.headers = {"x-subject-meta-*": 'test'}
        self.assertRaises(webob.exc.HTTPBadRequest,
    ...
    def test_subject_meta(self):
        subject_meta = {'x-subject-meta-size': 'test'}
        subject_meta_properties = {'properties': {'test': "test"}}
        actual = utils.subject_meta_to_http_headers(subject_meta)
        actual_test2 = utils.subject_meta_to_http_headers(
            subject_meta_properties)
        self.assertEqual({'x-subject-meta-x-subject-meta-size': u'test'}, actual)
        self.assertEqual({'x-subject-meta-property-test': u'test'},
                         actual_test2)
    ...
    def test_create_mashup_dict_with_different_core_custom_properties(self):
        ...
        mashup_dict = utils.create_mashup_dict(subject_meta)
        self.assertNotIn('properties', mashup_dict)
        self.assertEqual(subject_meta['properties']['test_key'],
    ...
    def test_create_mashup_dict_with_same_core_custom_properties(self):
        ...
        mashup_dict = utils.create_mashup_dict(subject_meta)
        self.assertNotIn('properties', mashup_dict)
        self.assertNotEqual(subject_meta['properties']['min_ram'],
                            mashup_dict['min_ram'])
        self.assertEqual(subject_meta['min_ram'], mashup_dict['min_ram'])
    ...
    def test_valid_fqdn(self):
        valid_inputs = ['localhost.localdomain',
                        ...
                        'img83.subject.xn--penstack-r74e.org']
    ...
    def test_valid_host_port_string(self):
        valid_pairs = ['10.11.12.13:80',
                       ...
                       'img83.subject.xn--penstack-r74e.org:13080']
