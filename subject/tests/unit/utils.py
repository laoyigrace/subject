...
import subject.db.simple.api as simple_db
...
def get_fake_request(path='', method='POST', is_admin=False, user=USER1,
    ...
    req.context = subject.context.RequestContext(**kwargs)
    return req
...
class FakeDB(object):
    ...
    def init_db():
        ...
        [simple_db.subject_create(None, subject) for subject in subjects]

        members = [
            {'subject_id': UUID1, 'member': TENANT1, 'can_share': True},
            {'subject_id': UUID1, 'member': TENANT2, 'can_share': False},
        ]
        [simple_db.subject_member_create(None, member) for member in members]

        simple_db.subject_tag_set_all(None, UUID1, ['ping', 'pong'])
...
class FakeStoreUtils(object):
    ...
    def delete_subject_location_from_backend(self, context,
                                           subject_id, location):
        if CONF.delayed_delete:
            self.schedule_delayed_delete_from_backend(context, subject_id,
                                                      location)
        else:
            self.safe_delete_from_backend(context, subject_id, location)
...
class FakeStoreAPI(object):
    ...
    def add_to_backend(self, conf, subject_id, data, size,
        ...
        self.data[subject_id] = (data, size)
        checksum = 'Z'
        return (subject_id, size, checksum, self.store_metadata)
...
class FakeGateway(object):
    def __init__(self, subject_factory=None, subject_member_factory=None,
                 subject_repo=None, task_factory=None, task_repo=None):
    ...
    def get_subject_factory(self, context):
        return self.subject_factory

    def get_subject_member_factory(self, context):
        return self.subject_member_factory

    def get_repo(self, context):
        return self.subject_repo
