...
class V2Token(object):
    ...
    def add_service(self, s_type, region_list=None):
        ...
        catalog = self.tok['access']['serviceCatalog']
        service_type = {"type": s_type, "name": "subject"}
        catalog.append(service_type)
        service = catalog[-1]
        endpoint_list = []
...
class TestKeystoneAuthPlugin(utils.BaseTestCase):
    ...
    def test_invalid_auth_url_v2(self):
        ...
        bad_creds = {
            'username': 'user1',
            'auth_url': 'http://localhost/badauthurl/v1.0/',
            'password': 'pass',
            'tenant': 'tenant1',
            'strategy': 'keystone',
            'region': 'RegionOne'
    ...
    def test_v1_auth(self):
        ...
        def fake_do_request(cls, url, method, headers=None, body=None):
            ...
            if (headers.get('X-Auth-User') != 'user1' or
                ...
                resp.status = 200
                resp.headers.update({"x-subject-management-url": "example.com"})
    ...
    def test_v2_auth(self):
        ...
        def fake_do_request(cls, url, method, headers=None, body=None):
            if (not url.rstrip('/').endswith('v1.0/tokens') or
                    url.count("2.0") != 1):
                self.fail("Invalid v1.0 token path (%s)" % url)
        ...
        mock_token = V2Token()
        mock_token.add_service('subject', ['RegionOne'])
        self.stubs.Set(auth.KeystoneStrategy, '_do_request', fake_do_request)
        ...
        no_region_creds = {
            'username': 'user1',
            'tenant': 'tenant-ok',
            'auth_url': 'http://localhost/redirect/v1.0/',
            'password': 'pass',
            'strategy': 'keystone'
        }
        ...
        # Add another subject service, with a different region
        mock_token.add_service('subject', ['RegionTwo'])
        ...
        mock_token = V2Token()
        mock_token.add_service('subject', ['RegionOne', 'RegionTwo'])
        ...
        mock_token = V2Token()
        mock_token.add_service('bad-subject', ['RegionOne'])
...
class TestImageMutability(utils.BaseTestCase):
    ...
    def _is_mutable(self, tenant, owner, is_admin=False):
        context = subject.context.RequestContext(tenant=tenant,
                                                 is_admin=is_admin)
        subject = self.subject_factory.new_subject(owner=owner)
        return authorization.is_subject_mutable(context, subject)
...
class TestImmutableImage(utils.BaseTestCase):
    ...
    def _test_change(self, attr, value):
        self.assertRaises(exception.Forbidden,
                          setattr, self.subject, attr, value)
        self.assertRaises(exception.Forbidden,
                          delattr, self.subject, attr)

    def test_change_id(self):
        self._test_change('subject_id', UUID2)
    ...
    def test_change_locations(self):
        ...
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.append, 'http://a/b/c')
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.extend, ['http://a/b/c'])
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.insert, 'foo')
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.pop)
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.remove, 'foo')
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.reverse)
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.sort)
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.__delitem__, 0)
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.__delslice__, 0, 2)
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.__setitem__, 0, 'foo')
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.__setslice__,
                          0, 2, ['foo', 'bar'])
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.__iadd__, 'foo')
        self.assertRaises(exception.Forbidden,
                          self.subject.locations.__imul__, 2)
    ...
    def test_change_tags(self):
        self.assertRaises(exception.Forbidden,
                          delattr, self.subject, 'tags')
        self.assertRaises(exception.Forbidden,
                          setattr, self.subject, 'tags', ['king', 'kong'])
        self.assertRaises(exception.Forbidden, self.subject.tags.pop)
        self.assertRaises(exception.Forbidden, self.subject.tags.clear)
        self.assertRaises(exception.Forbidden, self.subject.tags.add, 'king')
        self.assertRaises(exception.Forbidden, self.subject.tags.remove, 'ping')
        self.assertRaises(exception.Forbidden,
                          self.subject.tags.update, set(['king', 'kong']))
        self.assertRaises(exception.Forbidden,
                          self.subject.tags.intersection_update, set([]))
        self.assertRaises(exception.Forbidden,
                          self.subject.tags.difference_update, set([]))
        self.assertRaises(exception.Forbidden,
                          self.subject.tags.symmetric_difference_update,
                          set([]))

    def test_change_properties(self):
        self.assertRaises(exception.Forbidden,
                          delattr, self.subject, 'extra_properties')
        self.assertRaises(exception.Forbidden,
                          setattr, self.subject, 'extra_properties', {})
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.__delitem__, 'foo')
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.__setitem__, 'foo', 'b')
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.__setitem__, 'z', 'j')
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.pop)
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.popitem)
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.setdefault, 'p', 'j')
        self.assertRaises(exception.Forbidden,
                          self.subject.extra_properties.update, {})

    def test_delete(self):
        self.assertRaises(exception.Forbidden, self.subject.delete)

    def test_set_data(self):
        self.assertRaises(exception.Forbidden,
                          self.subject.set_data, 'blah', 4)

    def test_deactivate_subject(self):
        self.assertRaises(exception.Forbidden, self.subject.deactivate)

    def test_reactivate_subject(self):
...
class TestImageFactoryProxy(utils.BaseTestCase):
    ...
    def test_wrong_owner_cannot_be_set(self):
        self.assertRaises(exception.Forbidden,
                          self.subject_factory.new_subject, owner=TENANT2)

    def test_cannot_set_owner_to_none(self):
        self.assertRaises(exception.Forbidden,
                          self.subject_factory.new_subject, owner=None)

    def test_admin_can_set_any_owner(self):
        self.context.is_admin = True
        subject = self.subject_factory.new_subject(owner=TENANT2)
        self.assertEqual(TENANT2, subject.owner)

    def test_admin_can_set_owner_to_none(self):
        self.context.is_admin = True
        subject = self.subject_factory.new_subject(owner=None)
...
class TestImageRepoProxy(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.fixtures = [
            ...
            subject_factory.new_subject(owner=TENANT2, visibility='public'),
    ...
    def test_get_mutable_subject(self):
        subject = self.subject_repo.get(self.fixtures[0].subject_id)
        self.assertEqual(subject.subject_id, self.fixtures[0].subject_id)

    def test_get_immutable_subject(self):
        subject = self.subject_repo.get(self.fixtures[1].subject_id)
        self.assertRaises(exception.Forbidden,
                          setattr, subject, 'name', 'Vince')

    def test_list(self):
        subjects = self.subject_repo.list()
        self.assertEqual(subjects[0].subject_id, self.fixtures[0].subject_id)
        self.assertRaises(exception.Forbidden,
                          setattr, subjects[1], 'name', 'Wally')
        self.assertRaises(exception.Forbidden,
                          setattr, subjects[2], 'name', 'Calvin')
...
class TestTaskFactoryProxy(utils.BaseTestCase):
    def setUp(self):
        ...
        factory = subject.domain.TaskFactory()
        self.context = subject.context.RequestContext(tenant=TENANT1)
        self.context_owner_is_none = subject.context.RequestContext()
