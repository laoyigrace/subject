...
from subject.async.flows import convert
...
class TestImportTask(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        task_input = {
            "import_from": "http://cloud.foo/subject.raw",
    ...
    def test_convert_success(self):
        subject_convert = convert._Convert(self.task.task_id,
                                         self.task_type,
                                         self.img_repo)

        self.task_repo.get.return_value = self.task
        subject_id = mock.sentinel.subject_id
        subject = mock.MagicMock(subject_id=subject_id, virtual_size=None)
        self.img_repo.get.return_value = subject

        with mock.patch.object(processutils, 'execute') as exc_mock:
            exc_mock.return_value = ("", None)
            with mock.patch.object(os, 'rename') as rm_mock:
                rm_mock.return_value = None
                subject_convert.execute(subject, 'file:///test/path.raw')

                # NOTE(hemanthm): Asserting that the source format is passed
                # to qemu-utis to avoid inferring the subject format. This
    ...
    def test_convert_revert_success(self):
        ...
        with mock.patch.object(processutils, 'execute') as exc_mock:
            ...
            with mock.patch.object(os, 'remove') as rmtree_mock:
                ...
                subject_convert.revert(subject, 'file:///tmp/test')

    def test_import_flow_with_convert_and_introspect(self):
        self.config(engine_mode='serial',
                    group='taskflow_executor')

        subject = self.img_factory.new_subject(subject_id=UUID1,
                                             disk_format='raw',
                                             container_format='bare')
        ...
        subject_path = os.path.join(self.work_dir, subject.subject_id)
        ...
        def fake_execute(*args, **kwargs):
            ...
            open("%s.converted" % subject_path, 'a').close()
            return ("", None)
        ...
        with mock.patch.object(script_utils, 'get_subject_data_iter') as dmock:
            ...
            with mock.patch.object(processutils, 'execute') as exc_mock:
                ...
                self.assertEqual('qcow2', subject.disk_format)
                self.assertEqual(10737418240, subject.virtual_size)

                # NOTE(hemanthm): Asserting that the source format is passed
                # to qemu-utis to avoid inferring the subject format when
