...
from subject.async.flows import ovf_process
...
class TestOvfProcessTask(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        # The subject/tests/var dir containing sample ova packages used
        ...
        self.subject.container_format = 'ova'
        self.subject.context.is_admin = True
    ...
    def test_ovf_process_success(self, mock_find_file):
        ...
        oprocess = ovf_process._OVF_Process('task_id', 'ovf_proc',
                                            self.img_repo)
        self.assertEqual(ova_uri, oprocess.execute('test_subject_id', ova_uri))

        # Note that the extracted disk subject is overwritten onto the input ova
        # file
        with open(ova_file_path, 'rb') as disk_subject_file:
            content = disk_subject_file.read()
        # b'ABCD' is the exact contents of the disk subject file
        ...
        self.subject.extra_properties.update.assert_called_once_with(
            {'cim_pasd_InstructionSetExtensionName': 'DMTF:x86:VT-d'})
        self.assertEqual('bare', self.subject.container_format)
    ...
    def test_ovf_process_no_config_file(self, mock_find_file):
        ...
        # Note that the extracted disk subject is overwritten onto the input
        # ova file.
        with open(ova_file_path, 'rb') as disk_subject_file:
            content = disk_subject_file.read()
        # b'ABCD' is the exact contents of the disk subject file
    ...
    def test_ovf_process_not_admin(self, mock_find_file):
        ...
        self.subject.context.is_admin = False

        oprocess = ovf_process._OVF_Process('task_id', 'ovf_proc',
                                            self.img_repo)
        self.assertRaises(RuntimeError, oprocess.execute, 'test_subject_id',
                          ova_uri)
