...
class UtilsTestCase(test_utils.BaseTestCase):
    ...
    def test_empty_metadata_headers(self):
        ...
        headers = utils.subject_meta_to_http_headers(metadata)

        self.assertNotIn('x-subject-meta-snafu', headers)
        self.assertNotIn('x-subject-meta-uset', headers)
        self.assertNotIn('x-subject-meta-snafu', headers)
        self.assertNotIn('x-subject-meta-property-arch', headers)

        self.assertEqual('bar', headers.get('x-subject-meta-foo'))
        self.assertEqual('whistles', headers.get('x-subject-meta-bells'))
        self.assertEqual('', headers.get('x-subject-meta-empty'))
        self.assertEqual('', headers.get('x-subject-meta-property-distro'))
        self.assertEqual('nobody', headers.get('x-subject-meta-property-user'))
