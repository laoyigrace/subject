...
from subject.cmd import cache_manage
from subject.common import exception
import subject.common.utils
...
class TestGlanceCmdManage(test_utils.BaseTestCase):
    ...
    @mock.patch.object(subject.subject_cache.client.CacheClient,
                       'get_cached_subjects')
    @mock.patch.object(prettytable.PrettyTable, 'add_row')
    def test_list_cached_subjects(self, mock_row_create, mock_subjects):
        """
        Verify that list_cached() method correctly processes subjects with all
        filled data and subjects with not filled 'last_accessed' field.
        """

        mock_subjects.return_value = [
            {'last_accessed': float(0),
             'last_modified': float(1378985797.124511),
             'subject_id': '1', 'size': '128', 'hits': '1'},
            {'last_accessed': float(1378985797.124511),
             'last_modified': float(1378985797.124511),
             'subject_id': '2', 'size': '255', 'hits': '2'}]
        cache_manage.list_cached(mock.Mock(), '')

        self.assertEqual(len(mock_subjects.return_value),
                         mock_row_create.call_count)

    @mock.patch.object(subject.subject_cache.client.CacheClient,
                       'get_cached_subjects')
    def test_list_cached_subjects_empty(self, mock_subjects):
        """
        Verify that list_cached() method handles a case when no subjects are
    ...
    @mock.patch.object(subject.subject_cache.client.CacheClient,
                       'get_queued_subjects')
    @mock.patch.object(prettytable.PrettyTable, 'add_row')
    def test_list_queued_subjects(self, mock_row_create, mock_subjects):
        """Verify that list_queued() method correctly processes subjects."""
    ...
    @mock.patch.object(subject.subject_cache.client.CacheClient,
                       'get_queued_subjects')
    def test_list_queued_subjects_empty(self, mock_subjects):
        """
        Verify that list_queued() method handles a case when no subjects were
    ...
    def test_queue_subject_without_index(self):
        self.assertEqual(cache_manage.FAILURE,
                         cache_manage.queue_subject(mock.Mock(), []))

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_queue_subject_not_forced_not_confirmed(self,
        ...
        self.assertEqual(cache_manage.SUCCESS,
                         cache_manage.queue_subject(mock_options, ['img_id']))
        self.assertFalse(mock_client.called)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_queue_subject_not_forced_confirmed(self, mock_client, mock_confirm):
        ...
        self.assertIn(
            mock.call.mock_client().queue_subject_for_caching('img_id'),
            manager.mock_calls)

    def test_delete_cached_subject_without_index(self):
        self.assertEqual(cache_manage.FAILURE,
                         cache_manage.delete_cached_subject(mock.Mock(), []))

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_cached_subject_not_forced_not_confirmed(self,
                                                          mock_client,
                                                          mock_confirm):
        ...
        self.assertEqual(
            ...
            cache_manage.delete_cached_subject(mock_options, ['img_id']))
        self.assertFalse(mock_client.called)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_cached_subject_not_forced_confirmed(self, mock_client,
                                                      mock_confirm):
        ...
        self.assertIn(
            mock.call.mock_client().delete_cached_subject('img_id'),
            manager.mock_calls)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_cached_subjects_not_forced_not_confirmed(self,
                                                           mock_client,
                                                           mock_confirm):
        # options.forced set to False and delete confirmation set to False.
        ...
        self.assertEqual(
            cache_manage.SUCCESS,
            cache_manage.delete_all_cached_subjects(mock_options, None))
        self.assertFalse(mock_client.called)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_cached_subjects_not_forced_confirmed(self, mock_client,
                                                       mock_confirm):
        # options.forced set to False and delete confirmation set to True.
        ...
        self.assertTrue(mock_client.called)
        self.assertIn(
            mock.call.mock_client().delete_all_cached_subjects(),
            manager.mock_calls)

    def test_delete_queued_subject_without_index(self):
        self.assertEqual(cache_manage.FAILURE,
                         cache_manage.delete_queued_subject(mock.Mock(), []))

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_queued_subject_not_forced_not_confirmed(self,
                                                          mock_client,
                                                          mock_confirm):
        # options.forced set to False and delete confirmation set to False.
        ...
        self.assertEqual(
            cache_manage.SUCCESS,
            cache_manage.delete_queued_subject(mock_options, ['img_id']))
        self.assertFalse(mock_client.called)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_queued_subject_not_forced_confirmed(self, mock_client,
                                                      mock_confirm):
        # options.forced set to False and delete confirmation set to True.
        ...
        self.assertTrue(mock_client.called)
        self.assertIn(
            mock.call.mock_client().delete_queued_subject('img_id'),
            manager.mock_calls)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_queued_subjects_not_forced_not_confirmed(self,
                                                           mock_client,
                                                           mock_confirm):
        # options.forced set to False and delete confirmation set to False.
        ...
        self.assertEqual(
            cache_manage.SUCCESS,
            cache_manage.delete_all_queued_subjects(mock_options, None))
        self.assertFalse(mock_client.called)

    @mock.patch.object(subject.cmd.cache_manage, 'user_confirm')
    @mock.patch.object(subject.cmd.cache_manage, 'get_client')
    def test_delete_queued_subjects_not_forced_confirmed(self, mock_client,
                                                       mock_confirm):
        # options.forced set to False and delete confirmation set to True.
        mock_confirm.return_value = True
        mock_options = mock.Mock()
        ...
        self.assertTrue(mock_client.called)
        self.assertIn(
            mock.call.mock_client().delete_all_queued_subjects(),
            manager.mock_calls)
    ...
    @mock.patch.object(subject.cmd.cache_manage, 'lookup_command')
