...
import subject.api.v2.subject_actions as subject_actions
...
class TestImageActionsController(base.IsolatedUnitTest):
    ...
    def _get_fake_context(self, user=USER1, tenant=TENANT1, roles=None,
        ...
        context = subject.context.RequestContext(**kwargs)
        return context

    def _create_subject(self, status):
        self.subjects = [
        ...
        [self.db.subject_create(context, subject) for subject in self.subjects]

    def test_deactivate_from_active(self):
        self._create_subject('active')

        request = unit_test_utils.get_fake_request()
        self.controller.deactivate(request, UUID1)

        subject = self.db.subject_get(request.context, UUID1)

        self.assertEqual('deactivated', subject['status'])

    def test_deactivate_from_deactivated(self):
        self._create_subject('deactivated')
