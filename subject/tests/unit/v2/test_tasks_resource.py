...
def _domain_fixture(task_id, **kwargs):
    ...
    task = subject.domain.Task(**task_properties)
    return task

CONF = cfg.CONF
CONF.import_opt('task_time_to_live', 'subject.common.config', group='task')
...
class TestTasksController(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self._create_tasks()
        self.controller = subject.api.v2.tasks.TasksController(self.db,
                                                               self.policy,
                                                               self.notifier,
                                                               self.store)
        self.gateway = subject.gateway.Gateway(self.db, self.store,
    ...
    @mock.patch.object(subject.gateway.Gateway, 'get_task_factory')
    @mock.patch.object(subject.gateway.Gateway, 'get_task_executor_factory')
    @mock.patch.object(subject.gateway.Gateway, 'get_task_repo')
    ...
    @mock.patch('subject.common.scripts.utils.get_subject_data_iter')
    @mock.patch('subject.common.scripts.utils.validate_location_uri')
    def test_create_with_live_time(self, mock_validate_location_uri,
                                   mock_get_subject_data_iter):
        request = unit_test_utils.get_fake_request()
        task = {
            "type": "import",
            "input": {
        ...
        for import_from in wrong_import_from:
            ...
            if import_from.startswith("file:///"):
                msg = ("File based imports are not allowed. Please use a "
                       "non-local source of subject data.")
            else:
                supported = ['http', ]
    ...
    def test_create_with_properties_missed(self):
        ...
        self.assertEqual('failure', final_task.status)
        msg = "Input does not contain 'subject_properties' field"
        self.assertEqual(msg, final_task.message)
...
class TestTasksDeserializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.deserializer = subject.api.v2.tasks.RequestDeserializer()
...
class TestTasksSerializer(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.serializer = subject.api.v2.tasks.ResponseSerializer()
        self.fixtures = [
    ...
    def test_index(self):
        ...
        request = webob.Request.blank('/v1/tasks')
    ...
    def test_index_next_marker(self):
        ...
        self.assertEqual('/v1/tasks?marker=%s' % UUID2, output['next'])

    def test_index_carries_query_parameters(self):
        url = '/v1/tasks?limit=10&sort_key=id&sort_dir=asc'
        ...
        expected_url = '/v1/tasks?limit=10&sort_dir=asc&sort_key=id'
        self.assertEqual(unit_test_utils.sort_url_by_qs_keys(expected_url),
                         unit_test_utils.sort_url_by_qs_keys(output['first']))

        expect_next = '/v1/tasks?limit=10&marker=%s&sort_dir=asc&sort_key=id'
