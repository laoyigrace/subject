...
from subject.cmd import replicator as glance_replicator
...
IMG_RESPONSE_ACTIVE = {
    'content-length': '0',
    'property-subject_state': 'available',
    'min_ram': '0',
    'disk_format': 'aki',
    ...
    'size': '4660272',
    'property-subject_location': 'ubuntu-bucket/oneiric-server-cloudimg-amd64-'
                               'vmlinuz-generic.manifest.xml',
    'property-architecture': 'x86_64',
    'etag': 'f46cfe7fb3acaff49a3567031b9b53bb',
    'location': 'http://127.0.0.1:9292/v1/subjects/'
...
IMG_RESPONSE_QUEUED['location'] = ('http://127.0.0.1:9292/v1/subjects/'
...
class ImageServiceTestCase(test_utils.BaseTestCase):
    def test_rest_errors(self):
        ...
        for code, exc in [(400, webob.exc.HTTPBadRequest),
            ...
            self.assertRaises(exc, c.get_subject,
                              '5dcddce0-cba5-4f18-9cf4-9853c7b207a6')
    ...
    def test_rest_get_subjects(self):
        ...
        c.conn.prime_request('GET', 'v1/subjects/detail?is_public=None', '',
                             {'x-auth-token': 'noauth'},
                             200, jsonutils.dumps(resp), {})
        c.conn.prime_request('GET',
                             ('v1/subjects/detail?marker=%s&is_public=None'
                              % IMG_RESPONSE_QUEUED['id']),
                             '', {'x-auth-token': 'noauth'},
                             200, jsonutils.dumps({'subjects': []}), {})

        imgs = list(c.get_subjects())
    ...
    def test_rest_get_subject(self):
        ...
        c.conn.prime_request('GET',
                             ...
                             '', {'x-auth-token': 'noauth'},
                             200, subject_contents, IMG_RESPONSE_ACTIVE)

        body = c.get_subject(IMG_RESPONSE_ACTIVE['id'])
        self.assertEqual(subject_contents, body.read())

    def test_rest_header_list_to_dict(self):
        i = [('x-subject-meta-banana', 42),
             ('gerkin', 12),
    ...
    def test_rest_get_subject_meta(self):
        ...
        header = c.get_subject_meta(IMG_RESPONSE_ACTIVE['id'])
    ...
    def test_rest_dict_to_headers(self):
        ...
        self.assertIn('x-subject-meta-property-kernel_id', o)
        self.assertEqual(o['x-subject-meta-property-kernel_id'], '')
    ...
    def test_rest_add_subject(self):
        ...
        subject_meta_with_proto = {
            ...
            'Content-Length': len(subject_body)
        }

        for key in IMG_RESPONSE_ACTIVE:
            subject_meta_with_proto[
                'x-subject-meta-%s' % key] = IMG_RESPONSE_ACTIVE[key]

        c.conn.prime_request('POST', 'v1/subjects',
                             subject_body, subject_meta_with_proto,
                             200, '', IMG_RESPONSE_ACTIVE)

        headers, body = c.add_subject(IMG_RESPONSE_ACTIVE, subject_body)
    ...
    def test_rest_add_subject_meta(self):
        ...
        c.conn.prime_request('PUT', 'v1/subjects/%s' % subject_meta['id'],
                             '', subject_meta_headers, 200, '', '')
        headers, body = c.add_subject_meta(subject_meta)
...
class FakeImageService(object):
    ...
    def get_subject_meta(self, id):
        for img in FAKEIMAGES:
            if img['id'] == id:
                return img
        return {}

    def add_subject_meta(self, meta):
        return {'status': 200}, None

    def add_subject(self, meta, data):
        return {'status': 200}, None
...
def check_no_args(command, args):
    ...
    orig_img_service = glance_replicator.get_subject_service
    try:
        glance_replicator.get_subject_service = get_subject_service
        command(options, args)
    except TypeError as e:
        if str(e) == "Too few arguments.":
            no_args_error = True
    finally:
        glance_replicator.get_subject_service = orig_img_service
...
class ReplicationCommandsTestCase(test_utils.BaseTestCase):
    ...
    def test_replication_size(self):
        ...
        self.assertEqual(
            'Total size is 400 bytes (400.0 B) across 2 subjects',
            output
        )
    ...
    def test_replication_dump(self):
        ...
        self.addCleanup(setattr, glance_replicator,
                        'get_subject_service', orig_img_service)
        glance_replicator.get_subject_service = get_subject_service
    ...
    def test_replication_load(self):
        ...
        for img in FAKEIMAGES:
            cimg = copy.copy(img)
            # We need at least one subject where the stashed metadata on disk
            # is newer than what the fake has
            if cimg['id'] == '5dcddce0-cba5-4f18-9cf4-9853c7b207a6':
                cimg['extra'] = 'thisissomeextra'

            # This is an subject where the metadata change should be ignored
            if cimg['id'] == 'f4da1d2a-40e8-4710-b3aa-0222a4cc887b':
                cimg['dontrepl'] = 'thisisyetmoreextra'

            write_subject(cimg, 'kjdhfkjshdfkjhsdkfd')

        # And an subject which isn't on the destination at all
        new_id = str(uuid.uuid4())
        cimg['id'] = new_id
        write_subject(cimg, 'dskjfhskjhfkfdhksjdhf')

        # And an subject which isn't on the destination, but lacks subject
        # data
        new_id_missing_data = str(uuid.uuid4())
        cimg['id'] = new_id_missing_data
        write_subject(cimg, None)
