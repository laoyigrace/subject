...
class SubjectMemberRepoStub(object):
    ...
    def save(self, *args, **kwargs):
        return 'subject_member_from_save'

    def add(self, *args, **kwargs):
        return 'subject_member_from_add'

    def get(self, *args, **kwargs):
        return 'subject_member_from_get'

    def list(self, *args, **kwargs):
        return ['subject_members_from_list']


class TaskStub(subject.domain.TaskStub):
    def run(self, executor):
        pass


class Task(subject.domain.Task):
...
class TestNotifier(utils.BaseTestCase):
    ...
    def _test_load_strategy(self,
        ...
        mock_notifier.assert_called_with(nfier._transport,
                                         publisher_id='subject.localhost')
...
class TestSubjectNotifications(utils.BaseTestCase):
    ...
    def test_subject_save_notification(self):
        self.subject_repo_proxy.save(self.subject_proxy)
        ...
        self.assertEqual(self.subject.subject_id, output_log['payload']['id'])
        if 'location' in output_log['payload']:
            self.fail('Notification contained location field.')

    def test_subject_add_notification(self):
        self.subject_repo_proxy.add(self.subject_proxy)
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
    ...
    def test_subject_delete_notification(self):
        self.subject_repo_proxy.remove(self.subject_proxy)
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
    ...
    def test_subject_get_data_should_call_next_subject_get_data(self):
        with mock.patch.object(self.subject, 'get_data') as get_data_mock:
            self.subject_proxy.get_data()

            self.assertTrue(get_data_mock.called)

    def test_subject_get_data_notification(self):
        self.subject_proxy.size = 10
        data = ''.join(self.subject_proxy.get_data())
        self.assertEqual('0123456789', data)
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
        self.assertEqual('subject.send', output_log['event_type'])
        self.assertEqual(self.subject.subject_id,
                         output_log['payload']['subject_id'])
    ...
    def test_subject_get_data_size_mismatch(self):
        self.subject_proxy.size = 11
        list(self.subject_proxy.get_data())
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
    ...
    def test_subject_set_data_prepare_notification(self):
        ...
        def data_iterator():
            ...
            output_log = output_logs[0]
            self.assertEqual('INFO', output_log['notification_type'])
            self.assertEqual('subject.prepare', output_log['event_type'])
            self.assertEqual(self.subject.subject_id, output_log['payload']['id'])
            yield 'abcd'
            yield 'efgh'
            insurance['called'] = True

        self.subject_proxy.set_data(data_iterator(), 8)
        self.assertTrue(insurance['called'])

    def test_subject_set_data_upload_and_activate_notification(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
            yield 'fghij'

        self.subject_proxy.set_data(data_iterator(), 10)
        ...
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
        self.assertEqual('subject.upload', output_log['event_type'])
        self.assertEqual(self.subject.subject_id, output_log['payload']['id'])

        output_log = output_logs[1]
        self.assertEqual('INFO', output_log['notification_type'])
        self.assertEqual('subject.activate', output_log['event_type'])
        self.assertEqual(self.subject.subject_id, output_log['payload']['id'])

    def test_subject_set_data_storage_full(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_value_error(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_duplicate(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_storage_write_denied(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_forbidden(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_not_found(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_HTTP_error(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
    ...
    def test_subject_set_data_error(self):
        def data_iterator():
            self.notifier.log = []
            yield 'abcde'
...
class TestSubjectMemberNotifications(utils.BaseTestCase):
    ...
    def _assert_subject_member_with_notifier(self, output_log, deleted=False):
        self.assertEqual(self.subject_member.member_id,
                         output_log['payload']['member_id'])
        self.assertEqual(self.subject_member.subject_id,
                         output_log['payload']['subject_id'])
        self.assertEqual(self.subject_member.status,
                         output_log['payload']['status'])
        self.assertEqual(timeutils.isotime(self.subject_member.created_at),
                         output_log['payload']['created_at'])
        self.assertEqual(timeutils.isotime(self.subject_member.updated_at),
    ...
    def test_subject_member_add_notification(self):
        self.subject_member_repo_proxy.add(self.subject_member_proxy)
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
        self.assertEqual('subject.member.create', output_log['event_type'])
        self._assert_subject_member_with_notifier(output_log)

    def test_subject_member_save_notification(self):
        self.subject_member_repo_proxy.save(self.subject_member_proxy)
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
        self.assertEqual('subject.member.update', output_log['event_type'])
        self._assert_subject_member_with_notifier(output_log)

    def test_subject_member_delete_notification(self):
        self.subject_member_repo_proxy.remove(self.subject_member_proxy)
        output_logs = self.notifier.get_logs()
        self.assertEqual(1, len(output_logs))
        output_log = output_logs[0]
        self.assertEqual('INFO', output_log['notification_type'])
        self.assertEqual('subject.member.delete', output_log['event_type'])
        self._assert_subject_member_with_notifier(output_log, deleted=True)
...
class TestTaskNotifications(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.notifier = unit_test_utils.FakeNotifier()
        self.task_repo_proxy = subject.notifier.TaskRepoProxy(
            self.task_repo_stub,
            self.context,
            self.notifier
        )
        self.task_proxy = subject.notifier.TaskProxy(
            self.task,
            self.context,
            self.notifier
        )
        self.task_stub_proxy = subject.notifier.TaskStubProxy(
            self.task_stub,
            self.context,
            self.notifier
        )
    ...
    def test_task_run_notification(self):
        with mock.patch('subject.async.TaskExecutor') as mock_executor:
