...
def fork_exec(cmd,
    ...
    example: subject-api.  When subject-api starts it sets itself as
...
def start_http_server(subject_id, subject_data):
    def _get_http_handler_class(fixture):
        class StaticHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
            ...
            def do_HEAD(self):
                # reserve non_existing_subject_path for the cases where we expect
                # 404 from the server
                if 'non_existing_subject_path' in self.path:
    ...
    handler_class = _get_http_handler_class(subject_data)
...
class RegistryAPIMixIn(object):
    ...
    def get_api_response_ext(self, http_resp, url='/subjects', headers=None,
    ...
    def assertEqualImages(self, res, uuids, key='subjects', unjsonify=True):
        ...
        for i, value in enumerate(uuids):
            self.assertEqual(subjects[i]['id'], value)
