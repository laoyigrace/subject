...
class TestNamespaceProperties(functional.FunctionalTest):
    ...
    def test_properties_lifecycle(self):
        ...
        path = self._url('/v1/metadefs/namespaces/MyNamespace/properties'
                         '/property1')
        response = requests.get(path, headers=self._headers())
        self.assertEqual(404, response.status_code)
        ...
        path = self._url('/v1/metadefs/namespaces/%s/properties/%s' %
        ...
        path = self._url('/v1/metadefs/namespaces/%s/properties/%s%s' % (
