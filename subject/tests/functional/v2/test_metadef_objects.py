...
class TestMetadefObjects(functional.FunctionalTest):
    ...
    def test_metadata_objects_lifecycle(self):
        ...
        path = self._url('/v1/metadefs/namespaces/MyNamespace/objects/object1')
        response = requests.get(path, headers=self._headers())
        self.assertEqual(404, response.status_code)

        # Create a object
        path = self._url('/v1/metadefs/namespaces/MyNamespace/objects')
        headers = self._headers({'content-type': 'application/json'})
        ...
        path = self._url('/v1/metadefs/namespaces/%s/objects/%s' %
        ...
        expected_metadata_object = {
            ...
            "self": "/v1/metadefs/namespaces/%("
                    "namespace)s/objects/%(object)s" %
                    {'namespace': namespace_name,
                     'object': metadata_object_name},
            "schema": "v1/schemas/metadefs/object"
        }
