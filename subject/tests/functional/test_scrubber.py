...
class TestScrubber(functional.FunctionalTest):
    ...
    def _send_http_request(self, path, method, body=None):
        headers = {
            'x-subject-meta-name': 'test_subject',
    ...
    def test_delayed_delete(self):
        """
        test that subjects don't get deleted immediately and that the scrubber
        scrubs them
        """
        self.cleanup()
        ...
        self.assertEqual(200, response.status)
        self.assertEqual('pending_delete', response['x-subject-meta-status'])
    ...
    def test_scrubber_app(self):
        """
        test that the subject-scrubber script runs successfully when not in
        daemon mode
        """
        self.cleanup()
        ...
        # wait for the scrub time on the subject to pass
        time.sleep(self.api_server.scrub_time)

        # scrub subjects and make sure they get deleted
        exe_cmd = "%s -m subject.cmd.scrubber" % sys.executable
        cmd = ("%s --config-file %s" %
    ...
    def test_scrubber_delete_handles_exception(self):
        ...
        # ensure the subject is marked pending delete
        response, content = self._send_http_request(path, 'HEAD')
        self.assertEqual(200, response.status)
        self.assertEqual('pending_delete', response['x-subject-meta-status'])

        # Remove the file from the backend.
        file_path = os.path.join(self.api_server.subject_dir, subject['id'])
        os.remove(file_path)
    ...
    def wait_for_scrub(self, path, headers=None):
        ...
        for _ in range(wait_for / check_every):
            ...
            if (response['x-subject-meta-status'] == 'deleted' and
                    response['x-subject-meta-deleted'] == 'True'):
                break
            else:
                continue
        else:
            self.fail('subject was never scrubbed')
