...
from subject import tests as glance_tests
...
class Server(object):
    ...
    def __init__(self, test_dir, port, sock=None):
        ...
        self.show_subject_direct_url = False
    ...
    def create_database(self):
        ...
        if self.needs_database:
            conf_dir = os.path.join(self.test_dir, 'etc')
            utils.safe_mkdirs(conf_dir)
            conf_filepath = os.path.join(conf_dir, 'subject-manage.conf')
...
class ApiServer(Server):
    ...
    def __init__(self, test_dir, port, policy_file, delayed_delete=False,
        ...
        self.server_module = 'subject.cmd.%s' % self.server_name
        self.default_store = kwargs.get("default_store", "file")
        self.key_file = ""
        self.cert_file = ""
        self.metadata_encryption_key = "012345678901234567890123456789ab"
        self.subject_dir = os.path.join(self.test_dir, "subjects")
        self.pid_file = pid_file or os.path.join(self.test_dir, "api.pid")
        self.log_file = os.path.join(self.test_dir, "api.log")
        self.subject_size_cap = 1099511627776
        self.delayed_delete = delayed_delete
        self.owner_is_tenant = True
        self.workers = 0
        self.scrub_time = 5
        self.subject_cache_dir = os.path.join(self.test_dir,
                                            'cache')
        self.subject_cache_driver = 'sqlite'
        self.policy_file = policy_file
        self.policy_default_rule = 'default'
        self.property_protection_rule_format = 'roles'
        self.subject_member_quota = 10
        self.subject_property_quota = 10
        self.subject_tag_quota = 10
        self.subject_location_quota = 2
...
log_file = %(log_file)s
subject_size_cap = %(subject_size_cap)d
...
show_subject_direct_url = %(show_subject_direct_url)s
...
subject_member_quota=%(subject_member_quota)s
subject_property_quota=%(subject_property_quota)s
subject_tag_quota=%(subject_tag_quota)s
subject_location_quota=%(subject_location_quota)s
location_strategy=%(location_strategy)s
allow_additional_subject_properties = True
[oslo_policy]
...
[glance_store]
filesystem_store_datadir=%(subject_dir)s
default_store = %(default_store)s
"""
        self.paste_conf_base = """[pipeline:subject-api]
pipeline =
    cors
    healthcheck
    versionnegotiation
...
[pipeline:subject-api-cachemanagement]
pipeline =
    cors
    healthcheck
    versionnegotiation
...
paste.filter_factory =
 subject.api.middleware.version_negotiation:VersionNegotiationFilter.factory
...
[filter:cache_manage]
paste.filter_factory =
 subject.api.middleware.cache_manage:CacheManageFilter.factory
...
paste.filter_factory =
 subject.api.middleware.context:UnauthenticatedContextMiddleware.factory

[filter:fakeauth]
paste.filter_factory = subject.tests.utils:FakeAuthMiddleware.factory
...
flavor = %(deployment_flavor)s
"""
        self.paste_conf_base = """[pipeline:subject-registry]
pipeline = healthcheck unauthenticated-context registryapp

[pipeline:subject-registry-fakeauth]
pipeline = healthcheck fakeauth context registryapp

[pipeline:subject-registry-trusted-auth]
pipeline = healthcheck context registryapp

[app:registryapp]
paste.app_factory = subject.registry.api:API.factory
...
class FunctionalTest(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        conf_dir = os.path.join(self.test_dir, 'etc')
        utils.safe_mkdirs(conf_dir)
        self.copy_data_file('schema-subject.json', conf_dir)
    ...
    def start_servers(self, **kwargs):
        """
        Starts the API and Registry servers (subject-control api start
        & subject-control registry start) on unused ports.  subject-control
    ...
    def stop_server(self, server, name):
        ...
        subject-control stop method to gracefully shut the server down.
    ...
    def stop_servers(self):
        ...
        the subject-control stop method to gracefully shut the server down.
    ...
    def copy_data_file(self, file_name, dst_dir):
        src_file_name = os.path.join('subject/tests/etc', file_name)
