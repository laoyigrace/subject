...
CONF = cfg.CONF
CONF.import_opt('task_executor', 'subject.common.config', group='task')
...
class SubjectFactory(object):
    ...
    def _check_unexpected(self, kwargs):
        if kwargs:
            msg = _('new_subject() got unexpected keywords %s')
...
class Subject(object):
    ...
    valid_state_targets = {
        # Each key denotes a "current" state for the subject. Corresponding
        # values list the valid states to which we can jump from that "current"
        # state.
        # NOTE(flwang): In v1, we are deprecating the 'killed' status, so it's
        # allowed to restore subject from 'saving' to 'queued' so that upload
        # can be retried.
    ...
    def __init__(self, subject_id, status, created_at, updated_at, **kwargs):
    ...
    def status(self, status):
        ...
        # NOTE(flwang): Subject size should be cleared as long as the subject
    ...
    def deactivate(self):
        if self.status == 'active':
            ...
            LOG.debug("Not allowed to deactivate subject in status '%s'",
                      self.status)
            msg = (_("Not allowed to deactivate subject in status '%s'")
                   % self.status)
            raise exception.Forbidden(message=msg)
    ...
    def reactivate(self):
        if self.status == 'deactivated':
            ...
            LOG.debug("Not allowed to reactivate subject in status '%s'",
                      self.status)
            msg = (_("Not allowed to reactivate subject in status '%s'")
                   % self.status)
            raise exception.Forbidden(message=msg)
...
class TaskExecutorFactory(object):
    ...
    def __init__(self, task_repo, subject_repo, subject_factory):
    ...
    def new_task_executor(self, context):
        try:
            ...
            executor_cls = ('subject.async.%s_executor.'
