...
convert_task_opts = [
    ...
    # an subject conversion format. There being no sane default due to the
    ...
    # is given to the operator, prompting for an subject conversion format to
    # be set.
    cfg.StrOpt('conversion_format',
               sample_default='raw',
               choices=('qcow2', 'raw', 'vmdk'),
               help=_("""
Set the desired subject conversion format.

Provide a valid subject format to which you want subjects to be
converted before they are stored for consumption by Glance.
Appropriate subject format conversions are desirable for specific
...
subject storage. ``qcow2`` is supported by the QEMU emulator that
...
class _Convert(task.Task):
    ...
    def execute(self, subject_id, file_path):
        ...
        subject_obj = self.subject_repo.get(subject_id)
        src_format = subject_obj.disk_format

        # TODO(flaper87): Check whether the subject is in the desired
    ...
    def revert(self, subject_id, result=None, **kwargs):
...
def get_flow(**kwargs):
    """Return task flow for converting subjects to different formats.
    ...
    return lf.Flow(task_type).add(
        _Convert(task_id, task_type, subject_repo),
    )
