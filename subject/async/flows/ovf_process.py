...
class _OVF_Process(task.Task):
    """
    Extracts the single disk subject from an OVA tarball and saves it to the
    Glance subject store. It also parses the included OVF file for selected
    metadata which it then saves in the subject store as the previously saved
    ...
    def _get_extracted_file_path(self, subject_id):
        return os.path.join(CONF.task.work_dir,
                            "%s.extracted" % subject_id)
    ...
    def execute(self, subject_id, file_path):
        """
        :param subject_id: Id to use when storing extracted subject to Glance
            subject store. It is assumed that some other task has already
    ...
    def revert(self, subject_id, result, **kwargs):
        fs_path = self._get_extracted_file_path(subject_id)
...
class OVASubjectExtractor(object):
    ...
    def extract(self, ova):
        """Extracts disk subject and OVF file from OVA package

        Extracts a single disk subject and OVF from OVA tar archive and calls
        OVF parser method.
        ...
        for event, elem in ET.iterparse(ovf):
            if event == 'end':
                ...
                if tag == 'DiskSection':
                    ...
                    if len(disks) > 1:
                        """
                        Currently only single disk subject extraction is
                        supported.
                        FIXME(dramakri): Support multiple subjects in OVA package
                        """
...
def get_flow(**kwargs):
    ...
    return lf.Flow(task_type).add(
        _OVF_Process(task_id, task_type, subject_repo),
    )
